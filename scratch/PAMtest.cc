...
//VLC---------------------------------------------------------
  ...
  a -> SetPosition (Vector (0.0,0.0,50.0));
  ...
  //em2->setRes(0.28);
  em2->setM(4);
  double sym = log2(em2->getM()); //symbol rate constant which will be multipled by the base data rate since MPAM sends multiple bits per symbol
...
myfile << 50 - dist << " " << em2->getSER()*log2(em2->getM()) << std::endl;
myfile2 << 50 -dist << " " << em2->getSNR() << std::endl;
myfile3 << 50 - dist << " " << throughput << std::endl;
