/*
*   This script calculates and outputs the illuminance of an area.
*   The receiver moves about an area in the x and y direction with the z distance fixed.
*
...
std::ofstream myfile;
myfile.open("OOKIllumination1.dat");	//Outputs to file (file can be in .dat or .txt format)

std::ofstream myfile2;
myfile2.open("OOKIllumination2.dat");


for(double distx = -2.5 ; distx <= 2.5 ; distx+=.5){
for(double disty = -2.5 ; disty <= 2.5 ; disty+=.5){
...
//VLC---------------------------------------------------------
 OOKHelper OOK; // This helper makes the VLC channel that we are going to use
  ...
  a -> SetPosition (Vector (0.0,0.0,5.0));	//Set position of VLC transmitter
  b -> SetPosition (Vector (distx,disty,0.0));  //Set position of VLC receiver
  a ->SetAzimuth(0.0);
  b ->SetAzimuth(0.0);
  ...
  VPLM.setEfficacy(500, 500, 5000);	//(lower wavelength, upper wavelength, black body temperature)

  //Also initial conditions, but these are made in the error model since thats where the values are used to calculate BER
  
  em2->setNo(VPLM.wavelength_lower,VPLM.wavelength_upper,VPLM.temp,100e6,VPLM.GetPhotoDetectorArea(),VPLM.GetRxPower(a,b));
...
myfile << VPLM.calculateIlluminance(a, b) << " ";
myfile2 << distx << " " << disty << " " << VPLM.calculateIlluminance(a, b) << std::endl;
